...
def copy_XYZ_coords(guest_pdb_file, guest_sdf_file):
    # Copy XYZ Coordinates from docked structure onto prepared host_guest structure
    obConversion = openbabel.OBConversion()
    obConversion.SetInAndOutFormats("sdf", "xyz")
    mol = openbabel.OBMol()
    obConversion.ReadFile(mol,guest_sdf_file)  # Open Babel will uncompress automatically
    filename = os.path.splitext(guest_sdf_file)[0]
    obConversion.WriteFile(mol, f"{filename}.xyz")
    xyzfile = open(f"{filename}.xyz")
    xyz_content = xyzfile.readlines()

    with open(guest_pdb_file, 'r') as infile, open('host_guest_temp.txt', 'w') as outfile:
        i = 0
        for line in infile:
            if re.search('HETATM', line):
                ...
                i = i + 1
            outfile.write(line)
        os.rename("host_guest_temp.txt", guest_pdb_file)